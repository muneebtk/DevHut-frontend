...
function EditBlog() {
    ...
    let SubmitEditedData = ()=>{
        axios.put(`/blogs/edit_blog/${id}/`,form_data,config)
        .then((response)=>{
    ...
    let EditBlog = (id)=>{
        axios.get(`/blogs/edit_blog/${id}/`,config)
        .then((response)=>{
            if (response.status===200){
  ...
  return (
    <div>
        <Grid style={{ padding: "20px 0",backgroundColor:'silver', height:'100vh', }}>
          <Card
            className="card3"
            style={{
              margin: "auto",
          ...
          >
            ...
            <form onSubmit={EditBlogValidation} noValidate>
              <Grid style={{ padding: "5px 20px" }}>
                ...
                {/* <Typography style={{textAlign:'center'}} >Section 1</Typography><hr style={{width:'100px',}} align='start'/> */}
            ...
            <TextField name='subtitle1' type='text' style={{margin:''}} fullWidth={true}></TextField> */}
                ...
                <TextField value={category} name="category" type="text" onChange={(e) => setCategory(e.target.value)} fullWidth></TextField>
                {categoryError ? (
                  <Typography color="red" variant="caption">
                    {categoryError}
                  </Typography>
                ...
                <TextField  value={readTime} onChange={(e) => setReadTime(e.target.value)} name="read_time" type="text" fullWidth></TextField>
                {readTimeError ? (
                  <Typography color="red" variant="caption">
                    {readTimeError}
                  </Typography>
